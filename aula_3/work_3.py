...
def add_entry(dictionary, entry, **kwargs):
    ...
    is_collisions = collisions is not None
    val = entry['matricula']
    hash_key = module(val, mod)
    if is_collisions and len(dictionary[hash_key]) > 0:
...
with open("resultados.txt", "w") as file:
    for mod in modules:
        ...
        for entry_num in entries:
            ...
            file.write(f'MODULOS: {mod}\n')
            file.write(f'ITENS: {entry_num}\n')
            file.write(f'colisoes: {sum(collisions_list)}\n')
            file.write(f'tempo medio de busca {time_sum / len(test_list)}ms\n')
            ...
            file.write(f'desvio padrao: {desvio_padrao}\n')
            file.write('\n')
    ...
    for i in range(10):
        ...
        file.write(f'searching {item["matricula"]}...\n')
    ...
    file.write(f'tempo medio busca hash_table: {sum_hash_table / 10}ms\n')
    file.write(f'tempo medio busca list: {sum_common_list / 10}ms\n')
